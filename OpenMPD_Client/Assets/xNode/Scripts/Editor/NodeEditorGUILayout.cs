...
namespace XNodeEditor {
    ...
    public static class NodeEditorGUILayout {
        ...
        public static void PropertyField(SerializedProperty property, GUIContent label, XNode.NodePort port, bool includeChildren = true, params GUILayoutOption[] options) {
            ...
            //if (label == null)
            //{
            //    label = new GUIContent("hi " + property.displayName + " hi");
            //    label.tooltip = property.displayName;
            //}
            ////EditorGUIUtility.LookLikeControls(400);
            //if (port != null)
            //{
            //    Type thisnodetype = port.node.GetType();
            //    int nodeWidth = 0;
            //    if (!thisnodetype.TryGetAttributeWidth(out nodeWidth))
            //    {
            //        nodeWidth = 208;
            //    }
            ...
            //    s.CalcMinMaxWidth(label, out labelMin, out labelMax);
            //    EditorGUIUtility.labelWidth = Math.Min(nodeWidth / 2, labelMin);
            //    EditorGUIUtility.fieldWidth = Math.Min(nodeWidth / 2, nodeWidth - labelMin);

            //    Debug.Log(property.displayName + " nodewidth: " + nodeWidth + " minmax = " + labelMin + " " + labelMax + " " + EditorGUIUtility.fieldWidth + ":" + EditorGUIUtility.fieldWidth);
            //}

            // If property is not a port, display a regular property field
            if (port == null) EditorGUILayout.PropertyField(property, null, includeChildren);// new GUILayoutOption[] { GUILayout.ExpandWidth(true), GUILayout.MaxWidth(300), GUILayout.MinWidth(100) });
            else {
                Rect rect = new Rect();
                ...
                if (port.direction == XNode.NodePort.IO.Input) {
                    ...
                    switch (showBacking) {
                        case XNode.Node.ShowBackingValue.Unconnected:
                            ...
                            if (port.IsConnected) EditorGUILayout.LabelField(label != null ? label : new GUIContent(property.displayName, tooltip));// new GUILayoutOption[] { GUILayout.ExpandWidth(true), GUILayout.MaxWidth(300), GUILayout.MinWidth(100) }
                            // Display an editable property field if port is not connected
                            else EditorGUILayout.PropertyField(property, label, includeChildren); //new GUILayoutOption[] { GUILayout.MaxWidth(100), GUILayout.MinWidth(100) }
                            break;
                        ...
                        case XNode.Node.ShowBackingValue.Always:
                            ...
                            EditorGUILayout.PropertyField(property, label, includeChildren);//, new GUILayoutOption[] { GUILayout.MinWidth(5), GUILayout.MaxWidth(30)});
                            break;
                    }
        ...
        public static void PortField(GUIContent label, XNode.NodePort port, params GUILayoutOption[] options) {
            ...
            // If property is an output, display a text label and put a port handle on the right side
                ...
                // Display a label
                EditorGUILayout.LabelField(new GUIContent(content.text), NodeEditorResources.OutputPort, options);
