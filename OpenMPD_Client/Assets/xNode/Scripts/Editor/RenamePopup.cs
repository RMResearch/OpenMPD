        ...
        private void OnGUI() {
            ...
            if (input == null || input.Trim() == "") {
                if (GUILayout.Button("Revert to default") || (e.isKey && e.keyCode == KeyCode.Return)) {
                    ...
                    XNode.Node nodeTarget = target as XNode.Node;
                    NodeEditor.GetEditor(nodeTarget, NodeEditorWindow.current).OnRename();
                    string assetPath = AssetDatabase.GetAssetPath(target);
                    if (assetPath != "")
                    {
                        AssetDatabase.SetMainObject(nodeTarget.graph, assetPath);
